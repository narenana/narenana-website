...
Shared shell source: scripts/brand-shell.mjs and site/assets/family. Applications contain standalone copies so each can build/deploy independently. Keep those copies in sync when changing family navigation/fonts. The website homepage now uses the same family header as every other page (24 September 2026). It is generated by `familyNav({home:'', cta:{label:'Fly FPV', href:'https://sim.narenana.com', newTab:true}})` and pasted into `site/index.html`, and a suite test fails if the two differ. `cta` is an optional homepage-only button; without it, `familyNav()` output is unchanged, so the app copies need no update for this release. Log editorial pages regenerate through scripts/gen-content.mjs; main watch/methodology pages through scripts/seo-content.mjs. Nanawing 2 requires npm run build:static and its generated build mirror.
