...
const get = async url => {
  const r = await fetch(local ? localUrl(url) + (url.includes('?') ? '&' : '?') + 'audit=release2' : url, { signal: AbortSignal.timeout(20000) })
...
await Promise.all(Array.from({length:6}, async () => {
  while(next < urls.length) {
    ...
    try {
      ...
      const social = Object.fromEntries(['og:title','og:description','og:image','og:url','twitter:card'].map(k=>[k,meta(k) || null]))
